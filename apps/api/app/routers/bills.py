...
@router.get("/{bill_id}")
async def get_bill(bill_id: int, db: DbDep):
    bill = await bills_service.get_bill(bill_id, db)
    if bill is None:
...
async def get_bill_votes(bill_id: int, db: DbDep):
    """
    ...
    """
    vote = await votes_service.get_votes_for_bill(bill_id, db)
    if vote is None:
